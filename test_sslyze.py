...
def main() -> None:
    ...
    try:
        all_scan_requests = [
            ServerScanRequest(server_location=ServerNetworkLocation(hostname="172.29.120.196",port=443)),
            ServerScanRequest(server_location=ServerNetworkLocation(hostname="172.30.205.6",port=28181)),
        ]
    ...
    for server_scan_result in parsed_results.server_scan_results:
        ...
        if certinfo_attempt.status == ScanCommandAttemptStatusEnum.ERROR:
            ...
            for cert_deployment in certinfo_result.certificate_deployments:
                ...
                print(cert_deployment.received_certificate_chain[0].not_valid_before)
                print(cert_deployment.received_certificate_chain[0].not_valid_after)
            print("")
