...
def run():
    ...
    for server in server_list:
        ...
        open_ports = scan_ports.identify_open_ports(scanner, server['IP'], config.target_ports)
        print('Done!')
        if len(open_ports) > 0:
            if len(open_ports) == 1:
        ...
        if server['Ports'] is not None:
            ...
            for port in open_ports:
                    if port == 3306: # hardcoded until i can fix the sql ssl bug.
                        ...
                        certification = validate_certs.determine_certificate_information(validate_certs.extract_certificate(server['IP'], port))
                        ...
                        cert = {'NotBefore':before, 'NotAfter':after, 'Organization':org, 'CommonName':common}
                        if len(open_ports) == 1:
    ...
    process_files.write_csv(['hostname', 'IP', 'Ports', 'Certification'], results, 'checked_servers.csv')
    process_files.write_csv(['hostname', 'IP', 'Ports', 'Certification'], leftovers, 'skipped_servers.csv')
